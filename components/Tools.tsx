...
import { 
  ...
  Coins, DollarSign, PiggyBank, GraduationCap, Building2, Receipt, Plane, Users, Sliders, Grid, RotateCcw, Box, HeartPulse, List, Home, Wallet, Flame, Key, BarChart4
} from 'lucide-react';
...
const ResultCard = ({ title, amount, subtitle, color, symbol, privacyMode, chartData }: any) => {
...
const RentalYield = ({ symbol, userId, privacyMode }: any) => {
    ...
    return <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4"><InputGroup label="Monthly Rent" value={rent} onChange={setRent} symbol={symbol} /><InputGroup label="Monthly Exp" value={exp} onChange={setExp} symbol={symbol} /></div>
...
const DiscountCalculator = ({ symbol, userId, privacyMode }: any) => {
    ...
    return <div className="space-y-4"><InputGroup label="Price" value={p} onChange={setP} symbol={symbol} /><InputGroup label="Off %" value={d} onChange={setD} />
    <ResultCard title="Final Price" amount={p*(1-d/100)} subtitle={`Save ${symbol}${(p*d/100).toFixed(2)}`} color="pink" symbol={symbol} privacyMode={privacyMode} /></div>;
};
...
const TipCalculator = ({ symbol, userId, privacyMode }: any) => {
    ...
    return <div className="space-y-4"><InputGroup label="Bill" value={b} onChange={setB} symbol={symbol} /><InputGroup label="Tip %" value={t} onChange={setT} />
    <ResultCard title="Total" amount={b*(1+t/100)} subtitle={`Tip: ${symbol}${(b*t/100).toFixed(2)}`} color="teal" symbol={symbol} privacyMode={privacyMode} /></div>;
};
...
const CurrencyConverter = ({ userId, privacyMode }: any) => {
    ...
    return <div className="space-y-4"><InputGroup label="Amount (USD)" value={v} onChange={setV} symbol="$" /><InputGroup label="Rate" value={r} onChange={setR} />
    <ResultCard title="Converted" amount={v*r} subtitle="" color="cyan" symbol="" privacyMode={privacyMode} /></div>;
};
...
export const Tools: React.FC<ToolsProps> = ({ currency, userId, privacyMode }) => {
  ...
  const tools = [
    ...
    { id: 'dividend', name: 'Div. Yield', icon: BarChart4, color: 'green' },
